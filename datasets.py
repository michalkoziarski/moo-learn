    ...
    for i in range(5):
        for j in range(2):
            ...
            if noise_level > 0.0:
                ...
                majority_idx = [
                    i for i, cls in enumerate(train_set[1]) if cls == majority_class
                ]
                ...
                minority_class_after_conversion = classes[
                    np.argmin(sizes_after_conversion)
                ]
                majority_class_after_conversion = classes[
                    np.argmax(sizes_after_conversion)
                ]

                assert (
                    minority_class_after_conversion != majority_class_after_conversion
                )
